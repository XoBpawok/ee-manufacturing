...
Проста форма зворотного зв'язку: плаваюча кнопка (`FloatButton`) на всіх сторінках відкриває
модалку з формою (ім'я — опційно, повідомлення — обовʼязкове) і списком усіх відгуків нижче.
Зберігається в Supabase (таблиця `feedback`: `id`, `name`, `message`, `created_at`) — той самий
бекенд, що й `prices`. Anon може читати весь список і додавати запис (без update/delete з клієнта).
Без налаштованого Supabase кнопка не показується (`feedbackConfigured()` → `null`). Жодної пошти
чи зовнішніх сервісів. SQL — у `supabase/schema.sql`. API — `src/api/feedback.ts`.
